...
static s32 ov5645_read_reg(struct ov5645 *sensor, u16 reg, u8 *val)
{
	...
	if (i2c_master_send(sensor->i2c_client, au8RegBuf, 2) != 2) {
		dev_err(dev, "Write reg error: reg=%x\n", reg);
		return -1;
	}
			       ...
			       struct v4l2_dbg_register *reg)
{
	...
	ret = ov5645_read_reg(sensor, reg->reg, &val);
	if (!ret)
		reg->val = (__u64)val;
			       ...
			       const struct v4l2_dbg_register *reg)
{
	...
	return ov5645_write_reg(sensor, reg->reg, reg->val);
}
#endif
			...
			const struct i2c_device_id *id)
{
	...
	sensor->pwn_gpio = of_get_named_gpio(dev->of_node, "enable-gpios", 0);
	...
	/* request reset pin */
	sensor->rst_gpio = of_get_named_gpio(dev->of_node, "reset-gpios", 0);
